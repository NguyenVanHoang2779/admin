...
const httpErpNew = axios.create({
  baseURL: config.baseApiHrmNewUrl
})

httpErpNew.interceptors.request.use(
  (requestConfig) => {
...
httpErpNew.interceptors.response.use(function (response) {
  // Do something with response data
  retry = 0
  return response
}, handleError)

export default httpErpNew
