...
async function genToken (data, error) {
  if (!isOnAdminDomain) {
    ...
    if (gchatToken && retry === 0) {
      ...
      ++retry
      error.config.headers['gchat-token'] = gchatToken
...
const httpNewBag = axios.create({
  baseURL: config.baseApiNewBagUrl
})

httpNewBag.interceptors.request.use(
  (requestConfig) => {
    ...
    if (!isOnAdminDomain) {
      const gchatToken = GchatAuth.getToken()
      if (gchatToken) {
        requestConfig.headers['gchat-token'] = gchatToken
      }
    }
...
httpNewBag.interceptors.response.use(function (response) {
  // Do something with response data
  retry = 0
  return response
}, handleError)

export default httpNewBag
