...
export default {
  ...
  methods: {
    setList () {
      if (this.provinceIds) {
        this.provinceOption = []
        let listProvince = this.provinceIds.toString().split(',')
        let hasAll = true
    ...
    getAllProvince: function () {
      ...
      if (sessionStorage.getItem('allProvinces')) {
        ...
        if (!this.displayAll) {
          ...
          this.provinceOptions = (this.withRegions) ? [...region, ...listProvince] : [...listProvince]
        }
        this.setList()
        return
      }
        ...
        .then((res) => {
          if (res.data.success) {
            if (!this.displayAll) {
              ...
              this.provinceOptions = (this.withRegions) ? [...region, ...res.data.data] : [...res.data.data]
            }
            this.setList()
          }
        })
    ...
    getProvinceByRegion () {
      let region = this.region.constructor === Array ? this.region.join() : this.region

      addressService.getProvincesByRegionHr({ region })
        .then(res => {
          if (res.data.success) {
            this.provinceOptions = res.data.data
            this.triggerDuplicateChange = false
            this.setList()
            this.$emit('handleProvinceOption', this.provinceOptions.map(a => a.id))
          }
        })
        .catch((e) => {
          console.log(e)
