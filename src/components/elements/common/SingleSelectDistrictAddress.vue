...
export default {
  name: 'single-select-district-address',
  ...
  methods: {
    ...
    getListDistrict () {
      ...
      let cacheKey = `listDistrictsAddressByProvince-${this.provinceId}`
      if (this.isGetAll) {
        cacheKey = `listAllDistrictsAddress`
      }
      ...
      if (listDistrictsCached) {
        ...
        addressService.getDistrictAddressByProvinceId(params)
          .then((response) => {
