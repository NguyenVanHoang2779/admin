...
export default {
  name: 'multi-select-company-profile',
      ...
      CompanyProfileService.getCompanyProfileList(dataRequest)
        .then((res) => {
          if (res.data.success) {
            ...
            let tempProfileOption = []
            if (this.masterId) {
              this.masterId.forEach(e => {
                tempProfileOption.push(this.profileOptions.find((item) => item.id === e))
              })
            }
            console.log(tempProfileOption)
            this.profileOption = tempProfileOption
          }
        })
        .catch((e) => {
