  ...
  <b-row>
    <b-col sm="12">
      <multiselect
        ...
        label="name"
        track-by="id"
        placeholder="Nhập tên, số điện thoại hoặc mã shop để tìm kiếm"
        :options="asyncFound"
        :multiple="false"
        :searchable="true"
...
export default {
  ...
  watch: {
    asyncSelected () {
      this.$emit('onChange', this.asyncSelected, this.shopKey)
    }
  },
  methods: {
    asyncFind: debounce(function (query) {
      ...
      ManageLineService.getShopCarts(dataSend).then(response => {
