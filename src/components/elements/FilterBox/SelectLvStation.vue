  ...
  <div class="select-provinces">
    <b-row>
      <b-col class="col-md-3 btn-province-select">
        ...
        <b-button :variant="variantOther" @click="handerSelectOther(variantOther)" v-model="variantOther">Các kho khác</b-button>
      </b-col>
      <b-col class="col-md-9 province-multi-select">
        <multiselect class="multiselect-success" v-model="multipleValue" :options="multipleOptions" :multiple="true" :searchable="true" :max-height="250" label="name" track-by="name" placeholder="Chọn kho LV" selectedLabel="Đã chọn" :limit="10" selectLabel="Nhấn enter để chọn tỉnh" deselectLabel="Nhấn enter để bỏ chọn"/>
      </b-col>
    </b-row>
  </div>
</template>
...
export default {
  ...
  watch: {
    multipleValue: function (newMultipleValue, oldMultipleValue) {
      ...
      this.$emit('selectedStations', selectedValue)
    }
  },
  ...
  methods: {
    getLvStationList: function () {
      dashboardV5.getLvStation()
        .then((response) => {
