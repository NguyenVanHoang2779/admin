<template>
  <div>
    <div class="box-user d-flex justify-content-between">
      <div class="d-flex">
        <div class="avt-user">
          <img :src="order.profile.image_profile"
               alt="avatar user" width="52" height="52">
        </div>
        <div class="info-user ml-3">
          <div class="info-user-name">
            <b>{{ order.profile.fullname }} ({{ order.profile.username }})</b>
          </div>
          <div class="info-user-position">
            {{ order.profile.position_name }} - {{ order.profile.station }}
          </div>
        </div>
      </div>
      <div class="btn-chat">
        <div class="mb-1">
          <b-dropdown v-if="duty" size="md"  variant="link" toggle-class="text-decoration-none" class="p-0 dropdown-action-view" no-caret>
            ...
            <b-dropdown-item>
              <a name='stop-work-btn-view-profile-cod' id='stop-work-btn-view-profile-cod' @click="$emit('clickAction', {action: 'view_profile_cod', data: order.profile})" target="_blank">
                Xem profile
              </a>
            </b-dropdown-item>
          </b-dropdown>
        ...
        <div class="mt-1">
          <b-btn variant="outline-ghtk" class="btn-sm" name='stop-work-btn-call' id='stop-work-btn-call' @click="showModalPhone(order)">Gọi</b-btn>
        </div>
      </div>
    </div>
    ...
    <div class="btn-tags mb-1 mt-1">
      <div class="d-flex mt-2 content-tag">
        <div class="pl-1 pr-1 mr-1 mb-1 tag-danger" v-if="order.status === 'da_huy'">Đã hủy đề xuất</div>
        <div v-else>
        <div class="pl-1 pr-1 mr-1 mb-1 tag-default" v-if="!order.checkTick.tickOrder">Chưa duyệt đề xuất</div>

        <div class="pl-1 pr-1 mr-1 mb-1 tag-default" v-if="!order.checkTick.tickAsset">Chưa thu hồi CCDC</div>
        <div v-if="!order.checkTick.tickDeposit"><span class="pl-1 pr-1 mr-1 mb-1 tag-default">Chưa thanh toán cọc</span></div>
        <div v-if="!order.checkTick.tickInsurance"><span class="pl-1 pr-1 mr-1 mb-1 tag-default">Chưa chốt sổ BHXH</span></div>
        <div v-if="!order.checkTick.tickContract"><span class="pl-1 pr-1 mr-1 mb-1 tag-default">Chưa thanh lý HĐ</span></div>
        </div>
    ...
    <div class="profile-document d-flex">
      ...
      <template v-if="order.orderInsurance">
        <div class="position-relative file-relate mr-1 file-relate" v-if="order.orderInsurance.files"
             @click="showImage(order.orderInsurance.files, 'relate')" v-viewer >
          <div class="document-type-desc">
            Sổ BHXH - BH{{ order.orderInsurance.hrOrder.id }}
          </div>
          <div
            class="num-img-extend"
            v-show="order.orderInsurance.files.length - 1 > 1"
          >
            +{{ order.orderInsurance.files.length - 1 }}
          </div>
          <div v-for="(fileRelate, keyFile) in order.orderInsurance.files">
            <b-img
      ...
      <b-modal :id="'modal-number-phone-' + order.id" title="Gọi ứng viên" centered ref="phone-number" :hide-header-close="true"
        ...
        <div class="pl-3">
          <div class="mb-2">
            ...
            <span>{{ order.profile.fullname }}</span>
          </div>
          <div class="mb-2">
            <b class="font-weight-bold">Vị trí:</b>
            <span>{{ order.profile.position_name }}</span>
          </div>
          <div>
            <b class="font-weight-bold">Số điện thoại:</b>
            <span class="mb-2">{{ order.profile.tel || 'không có số điện thoại'}}</span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    showModalPhone (order) {
      this.getPhoneNumber(order.profile.user_id).then(tel => { this.$set(order.profile, 'tel', tel) })
      this.$bvModal.show('modal-number-phone-' + order.id)
    },
    ...
    async getPhoneNumber (userId) {
      try {
        const {data} = await profileService.getPhoneNumber({user_id: userId})
        if (data.success) {
