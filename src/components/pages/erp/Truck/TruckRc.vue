  ...
  <div class="col-md-12" style="margin: 0 !important;padding: 0 !important;">
    <b-form-row style="height: 27px;">
      <b-form-group class="col-md-5">
        ...
        <div style="width: 100px !important;margin-bottom: -25px;margin-left:-25px;" class="text-center">
          <b-btn @click="addRC()" class="btn btn-outline-success add-row-price btn-sm text-center" style="float: right;border-radius: 50%; font-size: 13px; padding: 0px !important; width: 20px; height: 20px;">
            <i class="fa fa-plus"/>
          </b-btn>
        </div>
      </b-form-group>
        ...
        <b-form-group class="col-md-5">
          <b-input-group v-if="optionsRc.length > 0">
            <multi-select-rc-code
              ...
              :lstRcCodes="tmpRc"
              :multiple="false"
              @handleChooseRcCode="handleChooseRcCode(index, ...arguments)"/>
          </b-input-group>
          <b-input-group v-else>
            <b-form-input :disabled="isPermissionEngineer" placeholder="Không tồn tại tài sản trong kho để chọn"></b-form-input>
          </b-input-group>
        </b-form-group>
        <b-col class="col-md-3" v-if="!checkDeleted(rcCode.is_active)">
          <ladda-btn @click.native="showDelete(rcCode.code)" data-style="zoom-out" class="ml-3 text-center py-2 btn-outline-primary" style="margin-top: 0.1rem !important;margin-left: 0 !important;border: solid 0.5px #e8e8e8;min-width: 100%;">
            Dừng hoạt động
          </ladda-btn>
        </b-col>
        <!--Hủy dừng-->
        <b-col class="col-md-3" v-else>
          <ladda-btn @click.native="revertRc(rcCode.code)" data-style="zoom-out" class="ml-3 text-center py-2 btn btn-outline-danger" style="margin-top: 0.1rem !important;margin-left: 0 !important;border: solid 0.5px #e8e8e8;min-width: 100%;">
            Hủy dừng
          </ladda-btn>
        </b-col>
        <b-col class="col-md-3" v-if="!checkDeleted(rcCode.is_active)">
...
export default {
  ...
  props: [
    'isPermissionEngineer', 'stationId', 'isRentInfo', 'lstRcCodes', 'orderTruck', 'indexPackage', 'isReloadRC'
  ],

  mounted () {
    this.options = this.packageChoose
    if (this.stationId) {
      this.getIdentifierCodes(this.stationId)
    }
  },

  watch: {
    lstRcCodes: function (newValue) {
    ...
    stationId: function (newValue, oldValue) {
      this.isReloadTruckRC = false
      this.getIdentifierCodes(newValue)
    },
  ...
  methods: {
    ...
    setRc: function () {
      this.tmpRc = this.lstRcCodes
      this.total = this.tmpRc.length
      if (this.total === 0) {
        this.addRC()
      }
    },
    handleChooseRcCode (index, optionRc, oldRc) {
      if (this.isReloadRC && this.countReload === 0) {
        this.countReload = 1
      }
      ...
      this.$emit('updateRc', {arrSelectedValidate: this.arrSelectedValidate})
    },
    ...
    deleteRcTruck (rcCode, index) {
      if (!rcCode) {
        commonHelper.showMessage('Không tồn tại mã xe RC ' + rcCode, 'warning')
        return
      }
