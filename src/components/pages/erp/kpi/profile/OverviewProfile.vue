<template>
  <div class="w-100" style="box-sizing: border-box; min-height: 500px">
    <div class="w-100 d-flex justify-content-between align-content-center mt-2 my-auto">
      ...
      <div class="d-inline-flex align-content-center">
        <div class="my-auto mr-2" v-if="isLog">
          <span class="font-weight-bold my-auto btn" style="margin: 0px 10px 0 10px;" @click="changeChannelLog('')">Tất cả {{ counterLog }}</span> |
          <span class="my-auto text-success btn" style="margin: 0px 10px 0 10px;" @click="changeChannelLog('package')">Đơn hàng {{ dataLogCounter.packageCounter }}</span> |
          <span class="my-auto text-success btn" style="margin: 0px 10px 0 10px;" @click="changeChannelLog('review')" >Đánh giá {{ dataLogCounter.reviewCounter }}</span> |
          <span class="my-auto text-success btn" style="margin: 0px 10px 0 10px;" @click="changeChannelLog('refund')" >Sự vụ {{ dataLogCounter.refundCounter }}</span>
        </div>
          <div>
    ...
    <div class="overview-content mt-4 pb-4" v-show="!isLog">
      <b-row v-if="(userInfo != null && Number(userInfo.Group.id) !== 4)">
        <b-col>
          <div>
            <h5 style="font-weight: bold">Lương, phạt & bồi hoàn</h5>
          </div>
        </b-col>
      </b-row>
      <div class="w-100" v-if="(userInfo != null && Number(userInfo.Group.id) !== 4)">
          <chart-bar :data-send="dataSend" :position="position"/>
        </div>
      <b-row>
        <b-col md="6">
          <div>
            <chart-penalty :data-send="dataSend"/>
          </div>
        </b-col>
        <b-col md="6">
          <div>
            <chart-refund :data-send="dataSend"/>
          </div>
        </b-col>
      </b-row>
      <chart-bar-and-line v-if="position === 'COD'" :data-send="dataSend" :position="position"/>
      <chart-percent-driver v-if="position === 'DRIVER'" :data-send="dataSend" :position="position"/>
      <chart-percent-xlhh v-if="position === 'XLHH'" :data-send="dataSend" :position="position"/>
    </div>
        ...
        <div class="list-item-wrapper" v-for="(data) in dataLogDetail">
          ...
          <div class="list-item">
            <div class="list-content">
              ...
              <span v-for="(strList) in data.str">
                <span v-if="strList.pkg"> <a :href="'/admin?alias=' + strList.value"
                                             target="_blank"> {{ strList.value }}</a></span>
                <span v-else>{{ strList.value }}</span>
              </span>
            </div>
          </div>
        </div>
...
import ChartRefund from './ChartRefund.vue'
import ChartPenalty from './ChartPenalty.vue'
import ChartBar from './ChartBar.vue'
import ChartBarAndLine from './ChartBarAndLine.vue'
import ChartPercentDriver from './ChartPercentDriver.vue'
import ChartPercentXlhh from './ChartPercentXlhh.vue'
...
export default {
  ...
  methods: {
    ...
    getLogCounter () {
      ...
      kpiProfilesService.getLogCounter(parmas)
        .then((res) => {
          if (res.data.success) {
            this.dataLogCounter = res.data.data
            this.counterLog = this.dataLogCounter.packageCounter + this.dataLogCounter.reviewCounter + this.dataLogCounter.refundCounter
          } else {
    ...
    getLogDetail () {
      ...
      kpiProfilesService.getLogDetail(parmas)
        .then((res) => {
          if (res.data.success) {
