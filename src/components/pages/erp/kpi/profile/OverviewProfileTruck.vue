<template>
  <div class="w-100" style="box-sizing: border-box; min-height: 500px">
    ...
    <div class="overview-content mt-4 pb-4" v-show="!isLog">
      ...
      <div class="w-100">
          <chart-line :data-send="dataSend" :position="position"/>
        </div>
      <chart-bar-truck :data-send="dataSend" :position="position"/>
    </div>
    ...
    <div class="overview-log mt-4 pb-4" v-if="!isLog">
      <h5 style="font-weight: bold">Sự vụ</h5>
      <div class="list-wrapper" id="incident" @scroll="onScroll">
...
import ChartLine from './ChartLineTruck.vue'
import ChartBarTruck from './ChartBarTruck.vue'
...
export default {
  ...
  computed: {
    ...
    default: () => {
      ...
      // Tháng trước đó 6 tháng = Tháng hiện tại - 5
      // Tháng hiện tại = d.getMonth() + 1
      let sixMonthBefore = d.getMonth() - 4
      let from = new Date(currentYear, sixMonthBefore, 0)
      if (sixMonthBefore <= 0) {
        sixMonthBefore = 12 + sixMonthBefore
        from = new Date(currentYear - 1, sixMonthBefore, 0)
      }
      return moment(from).format(`01/MM/YYYY`) + ' - ' + moment().format('DD/MM/YYYY')
    }
  },
  mounted () {
  ...
  methods: {
    ...
    getLogCounterTruck () {
      ...
      truckProfilesService.getLogCounterTruck(parmas)
        .then((res) => {
          if (res.data.success) {
            this.dataLogCounter = res.data.data
            this.counterLog = this.dataLogCounter.operationCounter + this.dataLogCounter.costCounter
    ...
    getLogDetail () {
      ...
      truckProfilesService.getLogDetail(parmas)
        .then((res) => {
          if (res.data.success) {
