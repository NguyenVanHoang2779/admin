<template>
  <div>
    ...
    <input
      ...
      :multiple="false"
      :accept="accept"
      @change="$emit('action', {action: 'new-file', data: {file: tmpFile}})"
      @input="addFile"
      :id="'tmpFile'"
      :name="'input_'"
    />
    <div class="m-0 d-flex justify-content-center" v-if="type !== 'view'">
      <label class="custom-label" :for="'tmpFile'" id="custom-file-label">
        <slot name="label">
          <div class="rounded cursor-pointer btn-ghtk px-5 py-1 font-large">Thêm tài liệu</div>
        </slot>
      </label>
    </div>
  </div>
