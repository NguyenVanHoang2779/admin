<template>
  <b-modal v-model="isShowModal" :id="id" static center modal-class="modal-xl" title="UPLOAD THÔNG TIN TÀI XẾ" @hide="onHideModal" :hide-footer="isHideFooter">
    <div class="position-relative dv-rent-driver" v-loading="importing">
      <p><a :href="'/admin/files/templates/Truck/template_rent_driver.xlsx'" id='rent-driver-btn-export-data' name='rent-driver-btn-export-data' target="_blank" style="color: #219653 !important;">Tải file mẫu</a>
      <b-file
    ...
    <div v-if="dataUploadError.length === 0">
      <div class="w-100 text-center" slot="modal-footer">
        <b-btn variant="success" @click="importRentCarDriver" v-if="!fileLog && !importing">Tải lên</b-btn>
      </div>
    </div>
...
export default {
  name: 'upload-rent-driver',

  props: {
    id: {
      default: 'upload-rent-driver'
    },
    isShow: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    importRentCarDriver () {
      ...
      this.importing = true
      CompanyProfileService.importRentCarDriver(data)
        .then(res => {
          if (res.data.success) {
...
<style scoped>
  ...
  .dv-rent-driver .alert-info {
    border-color: #fff;
    background-color: #fff;
  }
</style>
<style>
  #upload-rent-driver .modal-body button {
    margin-top: 40px;
    width: 180px;
    background-color: #00904A;
  }
  #upload-rent-driver .modal-header {
    display: flex;
    justify-content: center;
  }
</style>
