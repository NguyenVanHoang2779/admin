<template>
  <div class="content">
    <div class="headerD">
        <b-row>
          <b-col md="6">
            <h4 mt="2">Quản lý config COD</h4>
          </b-col>
          <b-col md="6">
            <b-button class="ml-3 customRouteL" variant="success" to="/salary/export-data-checkin-pkV2"><i class="fa fa-book"></i> Quản lý dữ liệu checkin</b-button>
            <router-link :to="{path: '/salary/config-checkin-cod-v3'}" class="btn btnManageData">+ Tạo config</router-link>
          </b-col>
        </b-row>
    </div>
    <div class="cardSearch">
      <b-row class="top topRowCustom align-items-center">
        <b-col md="3">
          <span class="spanSearch">Chọn phạm vi áp dụng:</span>
          <multiselect
            v-model="objectType"
        ...
        <b-col md="3" v-if="objectType.value == 1">
          <span class="spanSearch">Chọn kho:</span>
          <multiselect
            v-model="station"
        ...
        <b-col md="3" v-if="objectType.value == 1">
          <span class="spanSearch">Chọn công thức lương:</span>
          <multiselect
            v-model="formula"
        ...
        <b-col md="3" class="colUsername" v-if="objectType.value == 2">
          <span class="spanSearch spanUsername">Nhập tên người dùng:</span>
          <b-input v-model="username" :placeholder="`Nhập tên người dùng`"></b-input>
        </b-col>
        <b-col md="3" class="text-right">
          <b-button :disabled="isLoading" class="btn customRouteL btnSearch"  @click="getAllConfigPkbus">
            <i class="ion ion-md-search"></i> Tìm kiếm
          </b-button>
        </b-col>
      </b-row>
    </div>
    <b-card no-body class="cardInfo">
      <b-row>
          <b-col md="4">
            <span class="detailConfig">Đối tượng áp dụng: </span><span>{{ info.fullName }}</span>
          </b-col>
          <b-col md="4">
            <span class="detailConfig">Công thức lương: </span><span>{{ info.formula }}</span>
          </b-col>
          <b-col md="4">
            <span class="detailConfig">Tên công thức lương: </span><span style="font-size: 14px;">{{ info.formulaName }}</span>
          </b-col>
      </b-row>
    </b-card>
    <b-card v-for="(item, index) in listConfig.slice().reverse()" :key="index" no-body>
      <b-row class="mb-3 mt-4">
        ...
        <b-col md="3" style="margin-top: -0.8%">
          <router-link :to="{path: '/salary/config-checkin-cod-v3', query: {config_id: item.config_id, type: 1}}" class="btn btnDelete" style="background-color:#02b371 !important;" v-if="item.status_config == '1' && listConfig.length === 1"> + Tạo config tương tự</router-link>
      ...
      <b-row class="customBtnAdd">
        <router-link :to="{path: '/salary/config-checkin-cod-v3', query: {config_id: item.config_id, type: 2}}" class="btn customRouteLink" v-if="item.status_config == '2'">+ Chỉnh sửa</router-link>
      </b-row>
    </b-card >
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    if (param.hasOwnProperty('object_type') && param.hasOwnProperty('object_id')) {
      this.getAllConfigPkbus()
    }
    this.getListStation()
    this.getAllFormular()
  },
  ...
  methods: {
    ...
    getAllConfigPkbus () {
      this.isLoading = true
      let params = {}
      if (this.$route.query.hasOwnProperty('object_type') && this.$route.query.hasOwnProperty('object_id') && this.numberCall === 0) {
        params.objectType = this.$route.query.object_type
        params.objectId = this.$route.query.object_id
        if (Number(params.objectType) === 1) {
          params.formula = this.$route.query.hasOwnProperty('formula') ? this.$route.query.formula : null
        }
        this.numberCall = 1
      } else {
        params.objectType = this.objectType.value
        if (params.objectType === 2) {
          params.username = this.username
        } else {
          params.objectId = this.station.id ? this.station.id : null
          params.formula = this.formula.code ? this.formula.code : null
        }
      }
      SalaryServicePkbus.getAllConfigPkbus(params)
        .then((res) => {
          if (res.data.success) {
            this.listConfig = res.data.data.config
            let info = res.data.data.info
            this.info.id = info.id
            this.info.fullName = info.fullName
            if (Number(info.objectType) === 1) {
              this.info.fullName = 'Kho ' + this.info.fullName
            }
            this.info.formula = info.formula
            this.info.formulaName = info.formulaName
            this.isLoading = false
            if (res.data.data.config.length > 0) { return commonHelper.showMessage(res.data.message, 'success') }
    ...
    deleteConfig (config_id) {
      ...
      SalaryServicePkbus.deleteConfig(params)
        .then((res) => {
          if (res.data.success) {
            if ((this.objectType.value === 2 && this.username === '') || (this.objectType.value === 1 && (this.objectId === null || JSON.stringify(this.formula) === '{}'))) {
              this.numberCall = 0
            }
            this.getAllConfigPkbus()
