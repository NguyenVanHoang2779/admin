    ...
    <b-card no-body>
      <b-card-body>
        ...
        <b-row class="justify-content-md-center mb-2">
          ...
          <b-col md="1">
            <b-button variant="outline-success" @click="getLuongNV"><i class="sidenav-icon ion ion-md-search"></i>Tìm Kiếm</b-button>
          </b-col>
        </b-row>
  ...
  data () {
    return {
      ...
      fieldsDataConfigCheckin: [
        ...
        { key: 'address', label: 'Tên tỉnh', sortable: true },
        { key: 'checkin', label: 'Tổng công thực tế', sortable: true },
        { key: 'count_kpi_sunday', label: 'Công KPI ngày CN', sortable: true },
        { key: 'count_kpi_holiday', label: 'Công KPI ngày lễ', sortable: true },
        { key: 'kpi_raw', label: 'KPI thực làm', sortable: true },
        { key: 'reward_kpi', label: 'KPI thưởng', sortable: true },
        { key: 'weight_kpi', label: 'Kpi cân nặng', sortable: true },
        { key: 'kpi_raw_ot', label: 'KPI thực làm OT', sortable: true },
        { key: 'total_kpi', label: 'Tổng KPI', sortable: true },
        { key: 'average_kpi', label: 'KPI trung bình', sortable: true },
        { key: 'productivity_salary', label: 'Lương sản lượng', sortable: true },
        { key: 'manager_salary', label: 'Lương quản lý', sortable: true },
        { key: 'kpi_salary', label: 'Lương KPI', sortable: true },
        { key: 'trial_salary', label: 'Lương thử việc', sortable: true },
        { key: 'on_leave_salary', label: 'Lương phép', sortable: true },
  ...
  methods: {
    async exportToExcel () {
      ...
      try {
        ...
        let result = await SalaryService.exportSalaryNV(dataSend)
    ...
    async getListStations () {
      try {
        let res = await SalaryService.getListStationsNV()
    ...
    asyncFind: debounce(async function (query) {
      ...
      try {
        ...
        let res = await SalaryService.apiGetUserNV(dataSend)
    ...
    async getLuongNV () {
      ...
      try {
        ...
        let result = await SalaryService.getLuongNV(dataSend)
        if (result.data.success === false) {
          commonHelper.showMessage(result.data.message, 'failed')
        } else {
          commonHelper.showMessage(result.data.message, 'success')
