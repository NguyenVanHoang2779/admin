...
export default {
  ...
  methods: {
    ...
    getList: function (provinceIds) {
      if ((typeof provinceIds === 'undefined' || provinceIds === '-1') && localStorage.getItem('listStopPoints') !== null) {
        this.options = JSON.parse(localStorage.getItem('listStopPoints'))
        this.setList()
        return
      }
      let params = {
      ...
      salaryService.getListStopPointByProvince(params)
        .then((res) => {
          if (res.data.success) {
            ...
            this.setList()
            if (localStorage.getItem('listStopPoints') === null && (typeof provinceIds === 'undefined' || provinceIds === '-1')) {
              localStorage.setItem('listStopPoints', JSON.stringify(this.options))
            }
          }
        })
        .catch((e) => {
