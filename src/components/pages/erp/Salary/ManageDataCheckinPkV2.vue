<template>
  ...
  <b-card no-body>
    <b-card-body>
      <b-row>
        <h3>Quản Lý Dữ Liệu Checkin Pk</h3>
      </b-row>
      <hr>
      <b-row>
        <b-col sm="3">
      ...
      <b-row>
        ...
        <b-button class="btn btn-outline-info ml-3" @click="getDataCheckinByConditionNew">Lấy Dữ Liệu</b-button>
    ...
    <b-card-body>
      ...
      <b-modal id="manage-checkin-user" no-close-on-backdrop size="lg" title="Quản lý checkin"
               title-tag="h3" cancel-title="Hủy bỏ" static
               :hide-footer="true" @close="close"
      >
        <div>
          <span style="color: red">Để thêm ca checkin cho COD trước ngày 17-08-2022, thao tác tại
             <router-link :to="`export-data-checkin-pk`" style="color: #1c99e6"> đây </router-link>
          </span>
        </div>
        ...
        <b-row v-if="this.configTimeCheckin != null" class="mb-3">
          <b-row v-for="(configShift, index) in this.configTimeCheckin" :key="index" class="ml-3 mb-1">
            <b-btn-group class="float-right">
              <b-button variant="outline-secondary">Ca {{configShift.shift}}</b-button>
              <b-button variant="outline-secondary">{{configShift.point_name}}</b-button>
              <b-button variant="outline-secondary">Bắt đầu: {{(configShift.start.length > 5 ) ? configShift.start.slice(-8, -3) : configShift.start}}</b-button>
              <b-button variant="outline-secondary">Muộn: {{(configShift.late.length > 5 ) ? configShift.late.slice(-8, -3) : configShift.late}}</b-button>
              <b-button variant="outline-secondary">Kết thúc: {{(configShift.end.length > 5 ) ? configShift.end.slice(-8, -3) : configShift.end}}</b-button>
  ...
  methods: {
    ...
    handleStationSelected (ops) {
      if (ops !== null) {
        ...
        this.stationIds = ops ? ops.map(item => item.id).toString() : '' // gửi dạng string
      } else {
        this.stationIds = ''
      }
    },
    ...
    async checkPermission () {
      try {
        const res = await SalaryService.checkPermission()
        if (res.data.success) {
          if (res.data.data) {
            this.isPermission = res.data.data.is_permission
            return commonHelper.showMessage(res.data.message || 'Thành công', 'success')
          }
        } else {
          return commonHelper.showMessage(res.data.message || 'Có lỗi xảy ra', 'warning')
        }
      } catch (e) {
        if (e.response && e.response.data && e.response.data.message) {
          return commonHelper.showMessage(e.data.message || 'Có lỗi xảy ra', 'warning')
        }
      }
    },

    getDataCheckinByConditionNew: function () {
      if (this.from > this.to) {
        commonHelper.showMessage('Ngày bắt đầu phải nhỏ hơn ngày kết thúc!', 'warning')
        return
      }
      if (this.stationIds === null || this.stationIds === '') return commonHelper.showMessage('Vui Lòng Chọn Kho', 'warning')
      ...
      SalaryService.getAllDataCheckinByConditionNew({from, to, stationIds}).then(res => {
        if (res.data.success) {
    ...
    createCheckin () {
      ...
      dataSend.workShift = this.workShift.map((i) => this.configTimeCheckin[i])
      SalaryService.addCheckinUserNew(dataSend).then(res => {
        if (res.data.success) {
    ...
    getConfigCheckin () {
      ...
      SalaryService.getConfigUserNew(dataSend).then(res => {
        if (res.data.success) {
          this.configId = res.data.data.config_id
          this.configTimeCheckin = res.data.data.point_details
