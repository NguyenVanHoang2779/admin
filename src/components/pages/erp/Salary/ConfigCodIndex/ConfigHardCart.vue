<template>
  <b-card-body>
    <b-row>
      <h3 class="ml-3">Config Tuyến Khó</h3>
    </b-row>
    <br>
    <b-row>
      <b-col>
      ...
      <b-col>
        <multiselect
          ...
          v-model="indexHardCart"
          :options="indexHardCartOption"
          :multiple="true"
      ...
      <CreateConfigCart :model-pop-up-name="`Tạo config tuyến khó`"  :options="indexHardCartOption" :type-index="typeIndex" :title-create="`Chọn loại tuyến`" @getConfig="getConfig"/>
      <UpdateConfigCart :model-pop-up-name="`Update config tuyến khó`"  :options="indexHardCartOption" :type-index="typeIndex" :title-create="`Chọn loại tuyến`" @getConfig="getConfig" :data-update="dataUpdate"/>
    </b-row>
...
export default {
  ...
  data: () => ({
    ...
    fieldsDataConfig: [
      ...
      {key: 'index_number', label: 'Loại tuyến', sortable: true},
  ...
  methods: {
    ...
    getConfig (option) {
      ...
      let dataSend = {
        ...
        'num_index': this.indexHardCart ? this.indexHardCart.map((item) => item.id).toString() : null,
