<template>
  <b-card-body>
    <b-row>
      <h3 class="ml-3">Config đơn giá theo tuyến</h3>
    </b-row>
    <br>
    <b-row>
      <b-col>
      ...
      <b-col>
        <b-input v-model="indexNum" placeholder="Nhập đơn giá"></b-input>
      </b-col>
      <b-col md="1">
      ...
      <CreateConfigCartByInput :model-pop-up-name="`Tạo config đơn giá theo tuyến`" :type-index="typeIndex" :is-use-input-form="1" :title-create="`Nhập đơn giá (Đơn vị: VNĐ)`" @getConfig="getConfig"/>
      <UpdateCartByInput :model-pop-up-name="`Update Config đơn giá theo tuyến`" :type-index="typeIndex" :title-create="`Nhập đơn giá (Đơn vị: VNĐ)`" @getConfig="getConfig" :data-update="dataUpdate"/>
    </b-row>
...
export default {
  ...
  data: () => ({
    ...
    fieldsDataConfig: [
      ...
      {key: 'index_number', label: 'Đơn giá theo tuyến', sortable: true},
  ...
  methods: {
    ...
    getConfig (option) {
      ...
      let dataSend = {
        ...
        'type_index': 'cart_salary',
        'num_index': this.indexNum,
