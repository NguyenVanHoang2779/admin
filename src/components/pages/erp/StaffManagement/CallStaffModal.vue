<template>
  <div>
      ...
      <div v-if="user" v-loading="loading">
        ...
        <div>
          <b>Vị trí: </b><span>{{user.position.position_name}}</span>
        </div>
        <div>
...
export default {
  ...
  methods: {
    ...
    async getUserPhoneNumber () {
      try {
        this.loading = true
        const res = await staffManagementService.getStaffPhoneNumber({user_id: this.user.id, view_mode: this.options.viewMode})
