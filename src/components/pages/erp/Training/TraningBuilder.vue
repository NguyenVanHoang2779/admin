<template>
  <section class="workspace">
    ...
    <div class="right" v-loading="loading">
      <AddTrainingPage v-if="openning === 'add page'" @refreshTest="getTest" />
      <EditTrainingPage v-if="openning === 'edit page'" @refreshTest="getTest" :page="page"/>
      <AddTrainingQuestion
...
import PageTrainingItem from './PageTrainingItem'
import EditTrainingQuestion from './EditTrainingQuestion'
import EditTrainingPage from './EditTraningPage'
import AddTrainingQuestion from './AddTrainingQuestion'

export default {
  components: { AddTrainingQuestion, AddTrainingPage, PageTrainingItem, EditTrainingPage, EditTrainingQuestion },
  mounted () {
    this.getTest()
  },
  data () {
  ...
  methods: {
    ...
    async getTest () {
      try {
        this.loading = true
        const { data } = await trainingService.getTrainingTest({
    ...
    async createQuestion (reqData) {
      try {
        this.loading = true
        const { data } = await trainingService.createTrainingQuestion(reqData)
        if (data.success) {
          this.getTest()
    ...
    async editQuestion (reqData) {
      try {
        this.loading = true
        const { data } = await trainingService.editTrainingQuestion(reqData)
        if (data.success) {
          this.getTest()
          commonHelper.showMessage(data.message, 'success')
        } else commonHelper.showMessage(data.message, 'warning')
