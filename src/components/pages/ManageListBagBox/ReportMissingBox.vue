<template>
  <b-modal header-bg-variant="green" static no-close-on-esc no-close-on-backdrop :title="'Xác nhận báo mất hộp ' + erpAlias" v-model="showModal" hide-footer scrollable id="reportLostBoxDialog" @hidden="hideModal">
    <div class="text-center pb-3">
      Vui lòng chọn kho chịu trách nhiệm trước khi xác nhận mất hộp!
    </div>
    ...
    <div class="text-center pt-3">
      <b-button variant="primary" @click="doReportMissingBox" :disabled="isSubmitting">
  ...
  methods: {
    ...
    async doReportMissingBox () {
      try {
        ...
        const message = await manageListBagBoxService.reportMissingBox(this.erpAlias, this.station.id)
        this.isMissing = true
        this.hideModal()
