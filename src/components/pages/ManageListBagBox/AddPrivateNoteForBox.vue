<template>
  <b-modal header-class="justify-content-center" header-bg-variant="green" static no-close-on-esc hide-header-close
           :title="'Ghi chú hộp ' + erpAlias + ' (ID: #' + boxId + ' )'" v-model="showModal" hide-footer scrollable
  ...
  methods: {
    ...
    async addPrivateNote () {
      try {
        ...
        if (this.privateNote.trim().length < 10 || this.privateNote.trim().length > 255) {
          throw new Error('Vui lòng nhập ít nhất 10 kí tự và tối đa 255 kí tự!')
        }
        this.isSubmitting = true
        const message = await manageListBagBoxService.addPrivateNote(this.boxId, this.privateNote)
        this.hideModal()
