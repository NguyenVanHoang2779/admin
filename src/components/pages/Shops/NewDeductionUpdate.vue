<template>
    <div>
        <div>
            <div class="ui-block">
                <h4 class="ui-block-heading text-center">Điều chỉnh chiết khấu đặc biệt</h4>
            ...
            <form>
                ...
                <b-form-row>
                    <div class="align-content-center center">
                        ...
                        <ladda-btn
                                :loading="loading[1]"
                                @click.native="searchNewDDUpdate(pageData.currentPage, true, 1)"
                                data-style="zoom-out"
                                class="btn btn-success"
                        >
                        ...
                        <table class="table mb-0 table-bordered table-striped">
                            ...
                            <tr v-for="dataShops in listShops" class="tr-bill">
                                <td>
                                    ...
                                    <br/><br/>
                                    <span class="font-weight-bold">Gói chiết khấu: </span> {{ dataShops.NewDeduction.name }}<br/>
                                    <span class="font-weight-bold">Mã gói CK: </span> {{ dataShops.NewDeductionUpdate.new_dd_id }}<br/>
                                    <span class="font-weight-bold">Tháng chiết khấu: </span> {{ dataShops.NewDeductionUpdate.month_dd }}<br/>
                                    <br/>
                                    <span class="font-weight-bold">Thời gian upload: </span> {{ dataShops.NewDeductionUpdate.created }}<br/>
                                </td>
                                <td>
                                    <span class="font-weight-bold">Trạng thái: </span>
                                    <span class="badge align-text-bottom badge-danger" v-if="dataShops.NewDeductionUpdate.status == 1">Chưa chạy</span>
                                    <span class="badge align-text-bottom badge-warning" v-if="dataShops.NewDeductionUpdate.status == 2">Đang chạy</span>
                                    <span class="badge align-text-bottom badge-success" v-if="dataShops.NewDeductionUpdate.status == 3">Đã chạy xong</span>
                                    <span class="badge align-text-bottom badge-success" v-if="dataShops.NewDeductionResult.is_dd == 1">Được chiết khấu</span>
                                    <span class="badge align-text-bottom badge-danger" v-if="dataShops.NewDeductionResult.is_dd == 2">Không được chiết khấu</span>
                                    <br/><br/>
                                    <span class="font-weight-bold">Số đơn đủ điều kiện CK: </span> {{ dataShops.NewDeductionResult.number_pkg_deduction }}<br/>
                                </td>
                                <td>
                                    <ladda-btn
                                            @click.native="removeShop(dataShops.NewDeductionUpdate)"
                                            data-style="zoom-out"
                                            class="btn btn-success"
                                            v-if="canRunDD && dataShops.NewDeductionUpdate.status == 1"
                                    >
...
export default {
  ...
  methods: {
    ...
    removeShop: function (shop) {
      if (!confirm('Bạn chắc chắn muốn xóa thông tin điều chỉnh CKĐB của shop ' + shop.shop_code + '?')) {
        return
      }
      let self = this
      self.loadingExcel = true
      axios.get(config.baseApiUrl + 'new-deduction/removeNewDdUpdate/' + shop.id).then(response => {
        if (response.data.success) {
          self.loadingExcel = false
          self.searchNewDDUpdate(self.pageData.currentPage, true, 7)
          self.successRun = true
          self.dismissCountDownSE = self.dismissSeconds
          self.msgNotify = response.data.message ? response.data.message : 'Xóa thông tin CKĐB shop ' + shop.shop_code + ' thành công.'
        } else {
          self.loadingExcel = false
          alert('Không thể xóa thông tin CKĐB shop ' + shop.shop_code + ', vui lòng kiểm tra lại!')
        }
      })
    },
    runDD: function () {
      ...
      self.errorRun = false
      deductionService.runNewDeductionUpdate(dataSend).then(response => {
        if (response.data.success) {
    ...
    exportFileDD: function () {
      ...
      self.loadingExcel = true
      deductionService.exportFileNewDDUpdate(dataSend).then(response => {
        if (response.data.success) {
    ...
    exportFileZip: function () {
      ...
      self.loadingExcel = true
      deductionService.exportFileZipNewDDUpdate(dataSend).then(response => {
        if (response.data.success) {
          window.location.href = response.data.data
        } else {
          let msg = response.data.msg ? response.data.msg : 'Có lỗi xảy ra. Hãy thử lại nhé!'
    ...
    uploadFile: function (i) {
      ...
      self.uploadMsg = ''
      self.startLoading(i)
      deductionService.uploadFileNewDDUpdate(dataSend).then(response => {
        if (response.data.success) {
    ...
    searchNewDDUpdate: function (page, newSearchConditions, i) {
      let self = this
      let dataSend = {
        'month_deduction': self.customFormatterMonth(this.monthDeduction),
      ...
      this.startLoading(i)
      deductionService.searchNewDDUpdate(dataSend).then(response => {
        if (response.data.success) {
