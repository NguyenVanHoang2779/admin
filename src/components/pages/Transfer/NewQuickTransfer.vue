<template>
    <div>
        <div class="ui-block">
            <h4 class="ui-block-heading text-center">Chuyển Tiền Realtime</h4>
            <div>
                <form>
                    <div class="form-filter">
                        ...
                        <b-form-row>
                            ...
                            <div class="col-md-3">
                                <b-form-group label="Chọn trạng thái tiền sẽ chuyển">
                                    <select-money-status @selected="selected.moneyStatus = $event" ref="select_money_status"></select-money-status>
                                </b-form-group>
                            </div>
                        </b-form-row>
                    </div>
                    ...
                    <div class="table-result">
                        ...
                        <div class="table-responsive mb-4">
                            <table class="table mb-0 table-bordered table-striped">
                                ...
                                <tr>
                                    <th width="25%">Thông tin shop</th>
                                    <th width="10%">Tiền đã thu COD</th>
                                    <th width="10%">Tiền đã chốt ĐP</th>
                                    <th width="10%">Tiền đã nộp NH</th>
                                    <th width="10%">Tiền KT đã duyệt</th>
                                    <th width="15%">Tiền chuyển dự tính</th>
                                    <th width="15%">Tiền chuyển thực tế</th>
                                ...
                                <tr v-for="dataShops in listShops" class="tr-bill">
                                    <td>
                                        ...
                                        <br/>
                                        <span class="font-weight-bold">Trạng thái tiền sẽ chuyển: </span>
                                        <span class="font-weight-bold text-danger">{{ dataShops.Shop.transfer_type | transferType }}</span> <br/>
                                    </td>
                                    <td class="bill-info">
                                        <span class="font-weight-bold">Tổng tiền: </span><span>{{ parseFloat(dataShops.sum_bill_gen) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Số lượng ĐH: </span><span>{{ dataShops.count_bill_gen }}</span> <br/>
                                    </td>
                                    <td class="bill-info">
                                        <span class="font-weight-bold">Tổng tiền: </span><span>{{ parseFloat(dataShops.sum_committed) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Số lượng ĐH: </span><span>{{ dataShops.count_committed }}</span> <br/>
                                    </td>
                                    <td class="bill-info">
                                        <span class="font-weight-bold">Tổng tiền: </span><span>{{ parseFloat(dataShops.sum_confirmed) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Số lượng ĐH: </span><span>{{ dataShops.count_confirmed }}</span> <br/>
                                    </td>
                                    <td class="bill-info">
                                        <span class="font-weight-bold">Tổng tiền: </span><span>{{ parseFloat(dataShops.sum_act_cf) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Số lượng ĐH: </span><span>{{ dataShops.count_act_cf }}</span> <br/>
                                    </td>
                                    <td class="bill-info">
                                        ...
                                        <span class="font-weight-bold">Tổng tiền chuyển: </span><span>{{ parseFloat(dataShops.Shop.fn_cashback) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Số lượng ĐH: </span><span>{{ dataShops.Shop.count_pkg }}</span> <br/>
                                    </td>
                                    <td class="bill-info">
                                        <span class="font-weight-bold">Tổng thu hộ: </span><span>{{ parseFloat(dataShops.Transfer.total_pick_money) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Phí ship: </span><span>{{ parseFloat(dataShops.Transfer.total_ship_money) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Phí bảo hiểm: </span><span>{{ parseFloat(dataShops.Transfer.total_insurance) | toCurrency }}</span><br/>
                                        <span class="font-weight-bold">Shop trả trước: </span><span>{{ parseFloat(dataShops.Transfer.total_prepayment) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Phí chuyển hoàn: </span><span>{{ parseFloat(dataShops.Transfer.total_return_fee) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Khuyến mãi: </span><span>{{ parseFloat(dataShops.Transfer.total_discount) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Phí thay đổi địa chỉ giao: </span><span>{{ parseFloat(dataShops.Transfer.total_ch_addr_fee) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Phí chuyển khoản: </span><span>{{ parseFloat(dataShops.Transfer.transfer_fee) | toCurrency }}</span> <br/><br/>
                                        <span class="font-weight-bold">Tổng tiền chuyển: </span><span>{{ parseFloat(dataShops.Transfer.fn_cashback) | toCurrency }}</span> <br/>
                                        <span class="font-weight-bold">Số lượng ĐH: </span><span>{{ dataShops.Transfer.count_pkg }}</span> <br/>
                                    </td>
                                    <td class="bill-info text-center">
                                        <ladda-btn
                                                @click.native="showModal(dataShops.Shop.order, dataShops.Shop.transfer_fee, dataShops.Shop.code, dataShops.Shop.transfer_type)"
                                                data-style="zoom-out"
                                                class="btn btn-success"
                                        ...
                                        <br/><br/>
                                        <a href="javascript:void(0);" @click="exportPackageTransfer(dataShops.Shop.order)"><span>Xuất file</span></a>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
                        ...
                        <table class="table mb-0 table-bordered table-striped">
                            ...
                            <tr v-for="pkg in filterPkgData" class="tr-bill">
                                <td>
                                    ...
                                    <span class="font-weight-bold text-danger"> {{ parseFloat(pkg.cashback) | toCurrency }} </span><br/>
                                </td>
                                <td class="bill-info">
                                    ...
                                    <br/>
                                    <span class="font-weight-bold">Loại đơn: </span> {{ pkg.region_type | regionType }}<br/><br/>
                                    <span class="font-weight-bold">Tiền đã thu COD: </span> {{ parseFloat(pkg.bill_gen) | toCurrency }}<br/>
                                    <span class="font-weight-bold">Tiền đã chốt ĐP: </span> {{ parseFloat(pkg.committed) | toCurrency }}<br/>
                                    <span class="font-weight-bold">Tiền đã nộp NH: </span> {{ parseFloat(pkg.confirmed) | toCurrency }}<br/>
                                    <span class="font-weight-bold">Tiền KT đã duyệt: </span> {{ parseFloat(pkg.accountancy_cf) | toCurrency }}<br/>
                                    <span class="font-weight-bold">Tổng tiền đã thu: </span>
                                    <span class="font-weight-bold text-danger"> {{ parseFloat(pkg.total_money) | toCurrency }} </span><br/>
                                </td>
                                <td>
                                    <b-form-checkbox
...
import SelectMoneyStatus from 'components/elements/FilterBox/SelectMoneyStatus'
...
export default {
  ...
  methods: {
    resetForm: function () {
      ...
      this.$refs.select_money_status.reset()
    },
    showModal: function (shopOrder, transferFee, shopCode, transferType) {
      ...
      transferService.viewTransferRealtime(dataSend).then(response => {
    ...
    searchShop: function (page, newSearchConditions, i) {
      ...
      let dataSend = {
        ...
        'money_status': self.selected.moneyStatus,
        'page': page
      }
      ...
      this.startLoading(i)
      transferService.searchTransferRealtime(dataSend).then(response => {
        if (response.data.success) {
    ...
    exportPackageTransfer: function (shopOrder) {
      let self = this
      let dataSend = {
        'shop_order': shopOrder,
        'package_type': self.selected.packageTypes
      ...
      self.loadingTransfer = true
      transferService.exportPackageTransfer(dataSend).then(response => {
        if (response.data.success) {
  ...
  filters: {
    ...
    transferType: function (value) {
      if (!value) return ''
      if (value === '1') {
        return 'Tiền đã thu COD'
      } else if (value === '2') {
        return 'Tiền đã chốt ĐP'
      } else if (value === '3') {
        return 'Tiền đã nộp NH'
      } else if (value === '4') {
        return 'Tiền KT đã duyệt'
      } else {
        return ''
      }
    }
