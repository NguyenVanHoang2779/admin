<template>
  <div>
    <b-card class="echart-size-150" no-body>
      <h6 class="pt-3 pl-4">Thời gian TB lấy & giao hàng</h6>
...
export default {
  name: 'bar-operation-speed-pick-deliver',
  ...
  data: () => ({
    options: {
      ...
      legend: {
        data: ['chờ lấy', 'lấy xong', 'chờ giao', 'giao xong']
      },
      grid: {
        left: '3%',
        right: '4%',
  ...
  methods: {
    getData: function () {
      ...
      dashboardV4.getOperationSpeedPickDeliverBarData(params)
        .then((res) => {
          if (res.data.success) {
            let d1 = 0
            let d2 = 0
