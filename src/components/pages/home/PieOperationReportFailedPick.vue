<template>
  <div>
    <b-card class="default-pie-chart" no-body>
      <h6 class="pt-3 pl-4">Nguyên nhân không lấy được</h6>
...
export default {
  name: 'pie-operation-report-failed-pick',
  ...
  data: () => ({
    options: {
      ...
      legend: {
        ...
        x: 'left',
        data: [
          'Địa chỉ ngoài vùng phủ sóng',
          'Hàng không nhận vận chuyển',
          'NCC báo hủy',
          'NCC hoãn/delay 3 lần',
          'Đối tác hủy qua API',
          'Delay lấy hàng quá 3 lần',
          'Lý do khác'
        ]
      },
      series: [{
  ...
  methods: {
    getData: function () {
      ...
      dashboardV4.getOperationReportFailedPickPieChartData(params)
        .then((res) => {
          if (res.data.success) {
