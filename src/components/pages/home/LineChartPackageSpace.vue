...
export default {
  ...
  watch: {
    ...
    compareId: function (newValue, oldValue) {
      this.callAPI()
    }
  },

  methods: {
    getChartPackageSpaceData: function () {
      this.setDefault()
      let params = {
        ...
        compare: this.compareId
      }

      dashboardV4.getChartPackageSpaceData(params)
        .then((res) => {
          if (res.data.success) {
    ...
    callAPI: function () {
      if (this.firstTime === true) {
        if (this.dateRange && this.provinceIds && this.compareId) {
          this.getChartPackageSpaceData()
