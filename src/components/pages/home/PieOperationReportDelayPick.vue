<template>
  <div>
    <b-card class="default-pie-chart" no-body>
      <h6 class="pt-3 pl-4">Nguyên nhân delay lấy</h6>
...
export default {
  name: 'pie-operation-report-delay-pick',
  ...
  data: () => ({
    options: {
      ...
      legend: {
        ...
        x: 'left',
        data: [
          'Nhà cung cấp (NCC) hẹn lấy vào ca tiếp theo',
          'GHTK không liên lạc được với NCC',
          'NCC chưa có hàng',
          'NCC đổi địa chỉ',
          'NCC hẹn ngày lấy hàng',
          'GHTK quá tải, không lấy kịp',
          'Do điều kiện thời tiết, khách quan',
          'Lý do khác'
        ]
      },
      series: [{
  ...
  methods: {
    getData: function () {
      ...
      dashboardV4.getOperationReportDelayPickPieChartData(params)
        .then((res) => {
          if (res.data.success) {
