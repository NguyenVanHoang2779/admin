...
export default {
  ...
  watch: {
    option: function (newValue, oldValue) {
      ...
      if (isAll) {
        this.$emit('handleStation6hSelected', this.options)
      } else {
        this.$emit('handleStation6hSelected', newValue)
      }
    },
  ...
  methods: {
    getList: function (provinceIds) {
      ...
      dashboardV4.getStation6hListByProvinceIds(params)
        .then((res) => {
          if (res.data.success) {
            this.options = res.data.data.length > 0 ? [{id: -1, name: 'Tất cả kho 6h'}, ...res.data.data] : res.data.data
          }
        })
        .catch((e) => {
          console.log(e)
    ...
    handleInput: function (option) {
      ...
      if (isAll) {
        this.option = {id: -1, name: 'Tất cả kho 6h'}
      } else {
