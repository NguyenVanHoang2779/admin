...
export default {
  ...
  watch: {
    ...
    filterShopTimeId: function (newValue, oldValue) {
      this.callAPI()
    }
  },

  methods: {
    getChartShopTimeData: function () {
      this.setDefault()
      let params = {
        ...
        compare: this.filterShopTimeId
      }
      dashboardV4.getChartShopTimeData(params)
        .then((res) => {
          if (res.data.success) {
    ...
    callAPI: function () {
      if (this.firstTime === true) {
        if (this.dateRange && this.provinceIds && this.filterShopTimeId) {
          this.getChartShopTimeData()
