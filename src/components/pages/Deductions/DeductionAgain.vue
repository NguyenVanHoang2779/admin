<template>
    <div>
        <div>
            <div class="ui-block">
                <h4 class="ui-block-heading text-center">Chạy lại chiết khấu Shop</h4>
            ...
            <form>
                ...
                <b-form-row>
                    <div class="col-md-6" v-bind:disabled="btnDisablerunDD" v-if="!btnDisablerunDD && canRunDD">
                        <b-form-group label="Số thứ tự chiết khấu:">
                            <b-input type="text" placeholder="Nhập số thứ tự file chiết khấu" v-model="deductions.stt"/>
                        </b-form-group>
                    </div>
                </b-form-row>
                <b-form-row>
                        ...
                        <table class="table mb-0 table-bordered table-striped">
                            ...
                            <tr v-for="dataShops in listShops" class="tr-bill">
                                <td>
                                    ...
                                    <span class="badge align-text-bottom badge-success"> {{ dataShops.DeductionShopAgain.shop_code }} - {{ dataShops.Shop.name }}</span>
                                    <br/><br/>
                                    <span class="font-weight-bold">Gói chiết khấu: </span> {{ dataShops.Deduction.name }}<br/>
                                    <span class="font-weight-bold">Mã gói CK: </span> {{ dataShops.DeductionShopAgain.dd_id }}<br/>
                                    <span class="font-weight-bold">Thời gian áp dụng: </span> {{ dataShops.DeductionShopAgain.date_from }}<br/>
                                    <span class="font-weight-bold">Thời gian kết thúc: </span> {{ dataShops.DeductionShopAgain.date_to }}<br/>
                                    <br/>
                                    <span class="font-weight-bold">Thời gian upload: </span> {{ dataShops.DeductionShopAgain.created }}<br/>
                                </td>
                                <td>
                                    <span class="font-weight-bold">Trạng thái: </span>
                                    <span class="badge align-text-bottom badge-danger" v-if="dataShops.DeductionShopAgain.status == 1">Chưa chạy lại</span>
                                    <span class="badge align-text-bottom badge-warning" v-if="dataShops.DeductionShopAgain.status == 2">Đang chạy lại</span>
                                    <span class="badge align-text-bottom badge-success" v-if="dataShops.DeductionShopAgain.status == 3">Đã chạy xong</span>
                                    <span class="badge align-text-bottom badge-success" v-if="dataShops.DeductionShopAgain.is_dd == 1">Được chiết khấu</span>
                                    <span class="badge align-text-bottom badge-danger" v-if="dataShops.DeductionShopAgain.is_dd == 2">Không được chiết khấu</span>
                                    <br/><br/>
                                    <span class="font-weight-bold">Số đơn đủ điều kiện CK: </span> {{ dataShops.DeductionShopAgain.number_pkg }}<br/>
                                </td>
                                <td>
                                    <ladda-btn
                                            @click.native="removeShop(dataShops.DeductionShopAgain)"
                                            data-style="zoom-out"
                                            class="btn btn-success"
                                            v-if="canRunDD && dataShops.DeductionShopAgain.status == 1"
                                    >
...
export default {
  ...
  methods: {
    ...
    removeShop: function (shop) {
      if (!confirm('Bạn chắc chắn muốn xóa thông tin chiết khấu của shop ' + shop.shop_code + '?')) {
        return
      }
      let self = this
      self.loadingExcel = true
      axios.get(config.baseApiUrl + 'deduction-manager/removeShopDDAgain/' + shop.id).then(response => {
        if (response.data.success) {
          ...
          self.msgNotify = response.data.message ? response.data.message : 'Xóa thông tin chiết khấu shop ' + shop.shop_code + ' thành công.'
        } else {
          self.loadingExcel = false
          alert('Không thể xóa thông tin chiết khấu shop ' + shop.shop_code + ', vui lòng kiểm tra lại!')
        }
      })
    },
    runDD: function () {
      let self = this
      if (!confirm('Bạn muốn chạy lại chiết khấu ' + self.customFormatterMonth(this.monthDeduction) + ' cho các shop này?')) {
        return
      }
      let dataSend = {
      ...
      self.errorRun = false
      deductionService.runDeduction(dataSend).then(response => {
        if (response.data.success) {
          self.selectDeductionStatus = []
          self.selectDeductionStatus = { name: 'Đang chạy lại', dd_stt: '2' }
          self.searchShop(self.pageData.currentPage, true, 7)
          self.successRun = true
          self.dismissCountDownSE = self.dismissSeconds
          self.msgNotify = response.data.message ? response.data.message : 'Chạy lại chiết khấu thành công. Vui lòng chờ hệ thống hoàn tất quá trình chiết khấu!'
        } else {
          self.errorRun = true
    ...
    exportFileDD: function () {
      ...
      self.loadingExcel = true
      deductionService.exportFileDD(dataSend).then(response => {
        if (response.data.success) {
    ...
    exportFileZip: function () {
      ...
      self.loadingExcel = true
      deductionService.exportFileZip(dataSend).then(response => {
        if (response.data.success) {
    ...
    uploadFile: function (i) {
      ...
      self.uploadMsg = ''
      self.startLoading(i)
      deductionService.uploadFileDDAgain(dataSend).then(response => {
        if (response.data.success) {
    ...
    searchShop: function (page, newSearchConditions, i) {
      ...
      this.startLoading(i)
      deductionService.searchShop(dataSend).then(response => {
        if (response.data.success) {
    ...
    redirectDownload: function () {
      window.open('/adm/download-manage?type=ar')
    }
  },
  data: () => ({
    ...
    selectDeductionStatus: { name: 'Chưa chạy lại', dd_stt: '1' },
    DeductionStt: [
      { name: 'Chưa chạy lại', dd_stt: '1' },
      { name: 'Đang chạy lại', dd_stt: '2' },
