        ...
        <div class="row mb-4">
          ...
          <div class="col-md-6">
            <div class="row">
              ...
              <div class="col-md-4">
                <b-select v-model="searchData.formula" :options="listFormulas" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="table-responsive mb-4">
          <table class="table mb-0 table-bordered">
            ...
            <tbody v-if="listPayslips.length > 0">
              <tr v-for=" (payslip, index) in listPayslips">
                ...
                <td>{{payslip.EmpPayslip.month + '/' + payslip.EmpPayslip.year}}</td>
                <td v-numeral v-text="payslip.EmpContractSalary.trial_salary"></td>
                <td v-numeral v-text="payslip.EmpContractSalary.salary"></td>
                <td v-numeral v-text="payslip.EmpPayslip.net_salary"></td>
                <td>
                  <b-badge variant="outline-success" v-if="payslip.EmpPayslip.status === 'approved'">Đã duyệt</b-badge>
                  <b-badge variant="outline-secondary" v-if="payslip.EmpPayslip.status === 'pending'">Đang chờ duyệt</b-badge>
                </td>
                <td>
                  <router-link :to="getLinkPayslipDetail(payslip)" class="btn btn-outline-info btn-sm">Xem</router-link>
                  <b-btn v-if="payslip.EmpPayslip.status != 'approved'" class="btn btn-outline-danger btn-sm" @click="deletePayslip(payslip.EmpPayslip.id)">Xóa</b-btn>
                  <router-link v-if="payslip.EmpContractSalary.id" :to="getLinkEditContract(payslip)" class="btn btn-outline-info btn-sm">Thông tin hợp đồng</router-link>
                  <router-link v-else :to="getLinkCreateContract(payslip)" class="btn btn-outline-info btn-sm">Tạo hợp đồng</router-link>
                  <b-btn v-if="payslip.EmpPayslip.status === 'pending'" class="btn btn-outline-success btn-sm" @click="changePayslipStatus(payslip.EmpPayslip.id, payslip.EmpPayslip.status, index)">Duyệt</b-btn>
                  <!--<b-btn v-if="payslip.EmpPayslip.status === 'approved'" class="btn btn-outline-danger btn-sm" @click="changePayslipStatus(payslip.EmpPayslip.id, payslip.EmpPayslip.status, index)">Hủy duyệt</b-btn>-->
                </td>
              </tr>
            </tbody>
          </table>
...
import EcomBtnCreatePayslip from 'components/pages/Payslip/BtnCreatePayslip'

export default {
  name: 'ecom-payslip-manager',

  metaInfo: {
    title: 'Quản lý bảng lương nhân viên'
  },
  ...
  data () {
    return {
      ...
      listFormulas: this.getListFormulas(),
      listPayslips: [],
      totalItems: 0,
      currentPage: 1,
      perPage: 10,
  ...
  methods: {
    ...
    search (page = 1) {
      ...
      let dataSearch = {
        ...
        formula: this.searchData.formula
      }

      payslipService.getList(dataSearch).then((response) => {
    ...
    getLinkPayslipDetail (payslip) {
      return `/erp/payslip/detail/${payslip.EmpPayslip.id}`
    },

    getLinkEditContract (payslip) {
      return `/contract/detail/${payslip.EmpContract.id}`
    },

    getLinkCreateContract (payslip) {
      return `/contract/create/${payslip.User.username}`
    },
    ...
    getListFormulas () {
      let listFormulas = [{
        value: '',
        text: 'Chọn công thức lương COD'
      }]

      payslipService.getListSalaryFormulaCod().then((response) => {
        if (response.data.success) {
          let dataFormula = response.data.list_formula
          for (let formula in dataFormula) {
            if (!dataFormula.hasOwnProperty(formula)) continue
            listFormulas.push({
              value: formula,
              text: dataFormula[formula]
            })
          }
        }
      }).catch((error) => {
    ...
    exportExcel () {
      ...
      payslipService.requestExportFile(dataSearch).then((response) => {
        if (response.data.success) {
    ...
    checkExportFileComplete () {
      ...
      payslipService.checkExportFileComplete(data).then((response) => {
        if (response.data.success) {
    ...
    getDownloadExcelLink () {
      return payslipService.getDownloadExcelLink(this.fileExcel)
    },
    changePayslipStatus (id, status, index) {
      this.$startLoading()
      payslipService.changeStatusPayslip(id, status).then(response => {
        if (response.data.success) {
          console.log(this.listPayslips)
          if (status === 'pending') {
            this.listPayslips[index].EmpPayslip.status = 'approved'
          } else if (status === 'approved') {
            this.listPayslips[index].EmpPayslip.status = 'pending'
          }
          this.$notify({
            group: 'default',
            type: 'bg-success text-white',
        ...
        this.$notify({
          ...
          type: 'bg-warning text-dark',
          title: 'Thông báo',
          text: 'Có lỗi xảy ra trong quá trình thay đổi trạng thái payslip'
        })
      }).then(() => {
        this.$stopLoading()
      })
    ...
    deletePayslip (id) {
      ...
      this.$startLoading()
      payslipService.delete(id).then((response) => {
        if (response.data.success) {
          this.$notify({
            group: 'default',
            type: 'bg-success text-white',
    ...
    deleteByConditions () {
      ...
      this.$startLoading()
      payslipService.deleteByConditions(dataSearch).then((response) => {
        if (response.data.success) {
          this.$notify({
            group: 'default',
            type: 'bg-success text-white',
