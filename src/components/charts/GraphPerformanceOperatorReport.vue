<template>
  <div>
    <b-row>
      ...
      <b-col cols="12" md="3">
        <ecom-table-performance-operation-report :childRes="childRes"></ecom-table-performance-operation-report>
      </b-col>
    </b-row>
...
import EcomTablePerformanceOperationReport from 'components/tables/EcomTablePerformanceOperationReport'
...
export default {
  name: 'graph-performance-operator-report',

  components: {
    'vue-echart': ECharts,
    EcomTablePerformanceOperationReport,
  ...
  data: () => ({
    lineOptions: {
      ...
      color: ['#2f7ed8', '#0d233a', '#8bbc21', '#910000', '#1aadce', '#492970', '#f28f43', '#77a1e5'],
      legend: {
        data: ['Đơn hàng mới', 'Đã lấy hàng', 'Lấy đúng hạn 6h', 'Lấy đúng hạn 12h',
          'Đã giao hàng', 'Giao đúng hạn 6h', 'Giao đúng hạn 12h'
        ]
      },
      tooltip: {
        trigger: 'axis',
  ...
  methods: {
    getLineOptions: function () {
      ...
      params['reportType'] = 2
      dashboardV2.getPerformanceOperation(params)
        .then((response) => {
