...
const thirdParty = ['Active shops']
export default {
  name: 'GraphOperationByTimeReport',
  ...
  data: () => ({
    lineOptions: {
      ...
      color: ['#26B4FF', '#52F9B9', '#5DD1F4', '#02BC77', '#358FCF', '#75BFEB', '#BCDAEA', '#6DF2CC'],
      legend: {
        data: ['Đơn hàng hoàn thành', 'Đơn hàng hủy', 'Đơn hàng không giao được', 'Đơn hàng mới', 'Tỉ lệ hủy', 'Tỉ lệ hoàn thành', 'Tỉ lệ không giao được', 'Active shops']
      },
      tooltip: {
        trigger: 'axis',
        backgroundColor: '#fff',
  ...
  methods: {
    getLineOptions: function () {
      ...
      params['reportType'] = 2
      dashboardV5.getRealtimeOperationV5(params)
        .then((response) => {
