<template>
  <div>
    <h4 class="font-weight-bold py-3 mb-6 text-center">
      <span class="text-muted font-weight-light">Tỉnh</span>
    </h4>
    <ecom-table-province-picking-report :childRes="childRes"></ecom-table-province-picking-report>
    <div class="">
      <vue-echart :options="pieOptions"></vue-echart>
    </div>
     <!-- Pagination -->
...
import EcomTableProvincePickingReport from 'components/tables/EcomTableProvincePickingReport'
import EcomLoadingWanderingCubes from 'components/elements/loading/EcomLoadingWanderingCubes'

export default {
  name: 'pie-province-picking-report',

  components: {
    'vue-echart': ECharts,
    EcomTableProvincePickingReport,
  ...
  methods: {
    getPieOptions: function () {
      ...
      dashboardV2.getProvincePicking(params)
        .then((response) => {
